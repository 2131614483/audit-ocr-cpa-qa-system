...
# -*- coding: utf-8 -*-
"""
审计票据 & 凭证识别 — VLM 微调数据集生成脚本（Step 1: 生成 JSONL + 复制命令列表）

用法：
    python generate_vlm_dataset_jsonl.py --max_per_category 100 --output_dir ../dataset
    然后运行生成的 copy_files.sh / copy_files.bat 复制图片
"""
...
def generate_dataset(source_dir, output_dir, max_per_category=100, train_split=0.9, seed=42):
    ...
    for cat_idx, (category, image_files) in enumerate(categories.items(), 1):
        ...
        for img_idx, filename in enumerate(sampled_files, 1):
            ...
            dst_path_norm = dst_path.replace('\\', '/')

            # 记录复制命令（用于后续批量执行）
            copy_commands.append(f'cp "{src_path_norm}" "{dst_path_norm}"')
    ...
    print(f"\n{'='*60}")
    print(f"图片复制命令生成: {copied_count} 条")
    ...
    copy_script_path = os.path.join(output_dir, 'copy_files.sh')
    copy_bat_path = os.path.join(output_dir, 'copy_files.bat')
    ...
    # 写入复制脚本（Bash for Windows Git Bash）
    with open(copy_script_path, 'w', encoding='utf-8') as f:
        f.write('#!/bin/bash\n')
        f.write('# 自动生成的文件复制脚本\n')
        f.write(f'# 总计: {len(copy_commands)} 个文件\n\n')
        for cmd in copy_commands:
            f.write(cmd + '\n')

    # 写入 Windows batch 脚本
    with open(copy_bat_path, 'w', encoding='utf-8') as f:
        f.write('@echo off\n')
        f.write('REM 自动生成的文件复制脚本\n')
        f.write(f'REM 总计: {len(copy_commands)} 个文件\n\n')
        for cmd in copy_commands:
            # 将 cp 转换为 copy
            win_cmd = cmd.replace('cp ', 'copy ').replace('/', '\\')
            f.write(win_cmd + '\n')
    ...
    print(f"  - {val_path}")
    print(f"  - {meta_path}")
    print(f"  - {copy_script_path}")
    print(f"  - {copy_bat_path}")
    print(f"\n{'='*60}")
    print("JSONL 数据生成完成!")
    print(f"请运行以下命令复制图片:")
    print(f"  Bash:   bash {copy_script_path}")
    print(f"  CMD:    {copy_bat_path}")
    print(f"{'='*60}")

    return len(train_entries), len(val_entries), copy_commands
